...
begin
  ...
  -- Accounts, with the email the dashboard needs to actually reach someone.
  ...
  select jsonb_build_object(
    ...
    'logged_in_today', (
      select coalesce(jsonb_agg(jsonb_build_object(
               'user_id', user_id, 'email', email, 'logins', n, 'plan', plan,
               'signed_up', created_at, 'is_new', signup_day = v_today) order by n desc), '[]'::jsonb)
      from (
        select l.user_id, a.email, a.plan, a.created_at, a.signup_day, count(*) n
